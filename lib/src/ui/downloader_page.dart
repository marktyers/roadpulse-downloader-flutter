...
final class _DownloaderPageState extends State<DownloaderPage> {
    ...
    try {
      if (!Platform.isAndroid && delivery == DeliveryMethod.localSave) {
        final location = await selector.getSaveLocation(
          ...
          acceptedTypeGroups: const [selector.XTypeGroup(
            label: 'RoadPulse binary', extensions: ['rpb'])],
        );
        if (location != null) {
          await XFile.fromData(bytes, mimeType: 'application/octet-stream', name: _suggestedName)
              .saveTo(location.path);
          if (mounted) ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(content: Text('Saved ${controller.info!.recordCount} records.')));
        }
      } else {
        ...
        await SharePlus.instance.share(ShareParams(
          files: [XFile(file.path, mimeType: 'application/octet-stream', name: _suggestedName)],
          subject: 'RoadPulse Logger download ${controller.info!.deviceId}',
          text: 'RoadPulse Logger export (${controller.info!.recordCount} records).',
        ));
      }
    } catch (error) {
      if (mounted) ScaffoldMessenger.of(context).showSnackBar(
    ...
    builder: (context, _) => Scaffold(
      ...
      body: Center(child: ConstrainedBox(
        ...
        child: Padding(padding: const EdgeInsets.all(32), child: Column(
          ...
          children: [
            Icon(_icon, size: 64, color: Theme.of(context).colorScheme.primary),
            const SizedBox(height: 20),
            Text(controller.status, textAlign: TextAlign.center,
              style: Theme.of(context).textTheme.headlineSmall),
            const SizedBox(height: 10),
            Text(controller.detail, textAlign: TextAlign.center,
              style: Theme.of(context).textTheme.bodyLarge),
            if (controller.phase == DownloadPhase.downloading) ...[
              const SizedBox(height: 24), LinearProgressIndicator(value: controller.progress),
            ],
            if (_showPicker) ...[
              const SizedBox(height: 24),
              DropdownButtonFormField<LoggerDevice>(
                value: controller.selectedDevice,
                decoration: const InputDecoration(labelText: 'Serial device', border: OutlineInputBorder()),
                items: controller.devices.map((device) => DropdownMenuItem(
                  value: device, child: Text(device.label, overflow: TextOverflow.ellipsis))).toList(),
                onChanged: controller.selectDevice,
              ),
              const SizedBox(height: 12),
              FilledButton(onPressed: controller.selectedDevice == null ? null : controller.connect,
                child: const Text('Connect')),
            ],
            if (!Platform.isAndroid && controller.phase == DownloadPhase.ready) ...[
              const SizedBox(height: 22),
              SegmentedButton<DeliveryMethod>(
                segments: const [
                  ButtonSegment(value: DeliveryMethod.email,
                    icon: Icon(Icons.email_outlined), label: Text('Email')),
                  ButtonSegment(value: DeliveryMethod.localSave,
                    icon: Icon(Icons.save_alt), label: Text('Save locally')),
                ],
                selected: {delivery},
                onSelectionChanged: (value) => setState(() => delivery = value.single),
              ),
            ],
            if (controller.phase == DownloadPhase.ready) ...[
              ...
              FilledButton.icon(
                ...
                icon: Icon(delivery == DeliveryMethod.email || Platform.isAndroid
                    ? Icons.attach_email : Icons.save_alt),
                label: Text(delivering ? 'Preparing…'
                    : (delivery == DeliveryMethod.email || Platform.isAndroid
                        ? 'Create email' : 'Save RPB')),
              ),
            ],
            if (controller.phase == DownloadPhase.failed) ...[
              const SizedBox(height: 16),
              OutlinedButton(onPressed: controller.reset, child: const Text('Try again')),
            ],
            const SizedBox(height: 16),
  ...
  bool get _showPicker => controller.devices.length > 1 &&
      (controller.phase == DownloadPhase.waiting || controller.phase == DownloadPhase.failed);
