...
final class DownloadController extends ChangeNotifier {
  ...
  void start() {
    refreshDevices();
    _scanTimer =
        Timer.periodic(const Duration(seconds: 1), (_) => refreshDevices());
  }
  ...
  LoggerDevice? _preferred(List<LoggerDevice> found) {
    ...
    final likely = found.where((d) {
      ...
      return text.contains('usbmodem') ||
          text.contains('roadpulse') ||
          text.contains('cdc');
    }).toList();
  ...
  void selectDevice(LoggerDevice? device) {
  ...
  void _onBytes(Uint8List bytes) {
    ...
    try {
      ...
      if (expected != null && expected > 0) {
        ...
        detail =
            'Downloading: ${(_parser!.receivedPayloadBytes / 1000).toStringAsFixed(1)} of '
      ...
      if (result != null) {
        ...
        detail =
            '${(result.length / 1000).toStringAsFixed(1)} KB received. Flash was not changed.';
