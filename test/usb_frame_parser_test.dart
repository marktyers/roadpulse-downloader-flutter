...
void main() {
 group('UsbFrameParser', () {
  ...
  test('accepts CR, LF, tabs, and spaces between hex digits', () {
    final parser = UsbFrameParser();
    expect(parser.append('RPB_HEX_BEGIN 3\n0A\t 0b\r\n0C\nRPB_HEX_END'.codeUnits),
        Uint8List.fromList([10, 11, 12]));
  });

  test('accepts an empty payload', () {
    expect(UsbFrameParser().append('RPB_HEX_BEGIN 0\n\nRPB_HEX_END'.codeUnits), isEmpty);
  });
  ...
  test('decodes a large frame incrementally', () {
    ...
    for (final byte in source) {
      frame..write(digits[byte >> 4])..write(digits[byte & 15]);
    }
    frame.write('\nRPB_HEX_END\n');
    ...
    for (var start = 0; start < encoded.length; start += 4096) {
      decoded = parser.append(encoded.sublist(start,
          start + 4096 < encoded.length ? start + 4096 : encoded.length)) ?? decoded;
    }
